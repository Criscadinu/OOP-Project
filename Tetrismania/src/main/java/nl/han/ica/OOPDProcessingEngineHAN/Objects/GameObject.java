...
public abstract class GameObject implements IKeyInput, IMouseInput, PConstants {
	...
	/**
	 * Moves this object and sets the previous location. This function only works
	 * when this object has a speed.
	 */
	public void move() {
		if (speed > 0) {
	...
	/**
	 * Calculates the direction this object travels to based on the x speed and the
	 * y speed.
	 *
	...
	/**
	 * Sets the direction of this object in degrees Direction 0 points up,
	 * directions go clockwise, so 90 is right, etc.
	 *
	...
	/**
	 * Gets the direction of the objects movement in degrees. Direction 0 points up,
	 * directions go clockwise, so 90 is right, etc.
	 *
	...
	/**
	 * Sets both the direction and the speed of this object. Direction 0 points up,
	 * directions go clockwise, so 90 is right, etc.
	 *
	 ...
	 * <br />
	 * The decrease in speed is measured as a fraction, if you want a 5% decrease in
	 * speed per cycle of the game loop, use 0.05.
	 *
	 ...
	 * object from the left or right. The angle is calculated with the center x and
	 * center y of both objects.
	 * 
	 * @param object GameObject of which you would like to know whether its position
	 *               lays from your left or from your right.
	 * @return You'll be returned an angle between 0 and 360 degrees. 0 degrees
	 *         means the object is right above you, 90 degrees means the object is
	 *         at your right side, etc.
	 */
	...
	/**
	 * Use this function to get the distance between you and another object. First,
	 * this method calculates the angle from one object to another object. Second,
	 * this method checks if two objects intersect or not. Third, if there is no
	 * intersect, the angle between both objects determines what corners of two
	 * objects should be used to calculate the shortest distance between both
	 * objects. If two objects are in line (horizontally or vertically), the
	 * shortest distance between both objects is the offset in either x- or y-
	 * values of those corners. If not in line, Pythagoras is used to calculate the
	 * distance between two objects (better said; between two corners).
	 * 
	...
	public double getDistanceFrom(GameObject object) {
		...
		if (objectsIntersected(mainObjectTopLeftX, mainObjectTopRightX, mainObjectBottomRightX, mainObjectBottomLeftX,
			...
			distance = calculateDistanceTopRightAngle(mainObjectTopRightX, mainObjectTopRightY, otherObjectBottomLeftX,
					otherObjectBottomLeftY);
		}

		else if (otherObjectAtBottomRightOfMainObject(angle)) {
			distance = calculateDistanceBottomRightAngle(mainObjectBottomRightX, mainObjectBottomRightY,
					otherObjectTopLeftX, otherObjectTopLeftY);
		}

		else if (otherObjectAtBottomLeftOfMainObject(angle)) {
			distance = calculateDistanceBottomLeftAngle(mainObjectBottomLeftX, mainObjectBottomLeftY,
					otherObjectTopRightX, otherObjectTopRightY);
		}

		else if (otherObjectAtTopLeftOfMainObject(angle)) {
			distance = calculateDistanceTopLeftAngle(mainObjectTopLeftX, mainObjectTopLeftY, otherObjectBottomRightX,
					otherObjectBottomRightY);
		}
		return distance;
	}
	...
	private double calculateDistanceTopLeftAngle(float mainObjectTopLeftX, float mainObjectTopLeftY,
			float otherObjectBottomRightX, float otherObjectBottomRightY) {

		// If bottom right corner of other object is below the top left corner of the
		// main object.
		if (otherObjectBottomRightY > mainObjectTopLeftY) {
			// Then the shortest distance between both objects is the offset between both
			// x-values of those corners.
			return calculateDistance(mainObjectTopLeftX, otherObjectBottomRightX);
		}

		// If bottom right corner of other object is at right side of the top left
		// corner of the main object.
		else if (otherObjectBottomRightX > mainObjectTopLeftX) {
			// Then the shortest distance between both objects is the offset between both
			// y-values of those corners.
	...
	private double calculateDistanceBottomLeftAngle(float mainObjectBottomLeftX, float mainObjectBottomLeftY,
			float otherObjectTopRightX, float otherObjectTopRightY) {

		// If top right corner of other object is above left bottom corner of main
		// object
		...
		// If top right corner of other object is at the right side of the bottom left
		// corner of the main object.
	...
	private double calculateDistanceBottomRightAngle(float mainObjectBottomRightX, float mainObjectBottomRightY,
			float otherObjectTopLeftX, float otherObjectTopLeftY) {

		// If top left corner of other object is above right bottom corner of main
		// object
		...
		// If top left corner of other object is at the left side of the bottom right
		// corner of the main object.
		else if (otherObjectTopLeftX < mainObjectBottomRightX) {
			// The the shortest distance between both objects is the offset between both
			// y-values of those corners.
	...
	private double calculateDistanceTopRightAngle(float mainObjectTopRightX, float mainObjectTopRightY,
			float otherObjectBottomLeftX, float otherObjectBottomLeftY) {

		// If bottom left corner of other object is below the top right corner of the
		// main object
		...
		// If bottom left corner of the other object is at the left side of the top
		// right corner of the main object.
	...
	private boolean objectsIntersected(float mainObjectTopLeftX, float mainObjectTopRightX,
			float mainObjectBottomRightX, float mainObjectBottomLeftX, float mainObjectTopLeftY,
			float mainObjectTopRightY, float mainObjectBottomRightY, float mainObjectBottomLeftY,
			float otherObjectTopLeftX, float otherObjectTopRightX, float otherObjectBottomRightX,
			float otherObjectBottomLeftX, float otherObjectTopLeftY, float otherObjectTopRightY,
			float otherObjectBottomLeftY, float otherObjectBottomRightY) {
		return (otherObjectTopLeftX <= mainObjectTopRightX && otherObjectTopLeftX >= mainObjectTopLeftX
				&& otherObjectTopLeftY <= mainObjectBottomLeftY && otherObjectTopLeftY >= mainObjectTopLeftY)
				// or top right corner of other object is inside main object.
				|| (otherObjectTopRightX <= mainObjectTopRightX && otherObjectTopRightX >= mainObjectTopLeftX
						&& otherObjectTopRightY <= mainObjectBottomRightY
						&& otherObjectTopRightY >= mainObjectTopRightY)
				// or bottom left corner of other object is inside main object.
				|| (otherObjectBottomLeftX <= mainObjectTopRightX && otherObjectBottomLeftX >= mainObjectTopLeftX
						&& otherObjectBottomLeftY <= mainObjectBottomLeftY
						&& otherObjectBottomLeftY >= mainObjectTopLeftY)
				// or bottom right corner of other object is inside main object.
				|| (otherObjectBottomRightX <= mainObjectBottomRightX
						&& otherObjectBottomRightX >= mainObjectBottomLeftX
						&& otherObjectBottomRightY <= mainObjectBottomRightY
						&& otherObjectBottomRightY >= mainObjectTopRightY);
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see nl.han.ica.OOPDProcessingEngineHAN.UserInput.IKeyInput#keyPressed(int,
	 * char)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see nl.han.ica.OOPDProcessingEngineHAN.UserInput.IKeyInput#keyReleased(int,
	 * char)
	 */
	 ...
	 * 
	 * @see
	 * nl.han.ica.OOPDProcessingEngineHAN.UserInput.IMouseInput#mousePressed(int,
	 * int)
	 */
	 ...
	 * 
	 * @see
	 * nl.han.ica.OOPDProcessingEngineHAN.UserInput.IMouseInput#mouseReleased(int,
	 * int)
	 */
	 ...
	 * 
	 * @see
	 * nl.han.ica.OOPDProcessingEngineHAN.UserInput.IMouseInput#mouseClicked(int,
	 * int)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see nl.han.ica.OOPDProcessingEngineHAN.UserInput.IMouseInput#mouseMoved(int,
	 * int)
	 */
	 ...
	 * 
	 * @see
	 * nl.han.ica.OOPDProcessingEngineHAN.UserInput.IMouseInput#mouseDragged(int,
	 * int)
	 */
