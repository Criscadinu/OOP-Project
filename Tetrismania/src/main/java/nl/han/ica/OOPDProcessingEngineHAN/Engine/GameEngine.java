...
public abstract class GameEngine extends PApplet {
	...
	/**
	 * Creates a new GameEngine object, use a static main method and implement the
	 * following: PApplet.main(new String[]{"{YOUR.PACKAGENAME}.{YOUR.CLASSNAME}"});
	 */
	public GameEngine() {
	...
	/**
	 * The actual setup of the game-engine. Used to make sure the gameThread is
	 * started after setup has finished. This method is invoked by processing.
	 */
	public void setup() {
		setupGame();
		gameThread.start();
	 ...
	 * 
	 * @param gameObject The GameObject that will be added to the game. Should have
	...
	/**
	 * Updates every GameObject inside the GameEngine. Calls move method and checks
	 * for collision.
	 * 
	...
	public void updateGame() {
		...
		for (int i = 0; i < tempCollision.size(); i++) {
			...
			if (tempCollision.get(i) instanceof ICollidableWithGameObjects) {
				Vector collidedGameobjects = CollidingHelper.calculateGameObjectCollisions(tempCollision.get(i),
						tempCollision);
				if (!collidedGameobjects.isEmpty())
					((ICollidableWithGameObjects) tempCollision.get(i))
							.gameObjectCollisionOccurred(collidedGameobjects);
			}
	...
	/**
	 * Gets and calculates the relative mouse position with the location of world
	 * (TileMap). A negative value means the mouse position is detected outside the
	 * world map.
	 * 
	 * @param x
	 * @param y
	...
	/**
	 * Creates a new PGraphics (canvas) object which can only made by the PApplet.
	 * (this method is used by the GameEngine)
	 * 
	 * @param width
	 * @param height
	 * @return PGraphics
	...
	/**
	 * The CollidingHelper can calculate collision detection with GameObjects and
	 * Tiles.
	 */
	private static class CollidingHelper {
		 ...
		 * @return Vector<GameObject>
		 */
		private static Vector<GameObject> calculateGameObjectCollisions(GameObject gameObject,
			...
			for (int i = 0; i < allGameObjects.size(); i++) {
				if (!allGameObjects.get(i).equals(gameObject)) {
					otherGameObjectRectangle = new Rectangle((int) allGameObjects.get(i).getX(),
							(int) allGameObjects.get(i).getY(), (int) allGameObjects.get(i).getWidth(),
							(int) allGameObjects.get(i).getHeight());
					collidedObjects.addAll(checkCollisionBetweenObjects(gameObject, allGameObjects.get(i),
							gameObjectRectangle, otherGameObjectRectangle));
				}
			}
			...
			for (int i = 0; i < tileMap.getTileMap().length; i++) {
				for (int j = 0; j < tileMap.getTileMap()[i].length; j++) {
					if (!tilemapIndexHasEmptyTile(tileMap, i, j)) {
						...
						collidedTiles.addAll(checkCollisionBetweenTileAndObject(gameObject,
								tileMap.getTileOnIndex(j, i), tileRectangle, gameObjectRectangle));
					}
				}
			}
		...
		/**
		 * This method returns an integer between 0 and 3 to define which side of the
		 * Tile has been hit by the GameObject.
		 * 
		 * @param gameObject
		 * @param tile
		 * @return int
		...
		private static int getCollidedTileSide(Rectangle gameObject, Rectangle tile) {
			...
			float angleGameObjectToTile = getAngleBetweenPoints(checkpointObjectX, checkpointObjectY, centerTileX,
					centerTileY);
		 ...
		 * @return int
		 */
		private static int getCollidedTileSide(Rectangle gameObject, int centerTileX, int centerTileY,
				float angleGameObjectToTile) {
		...
		/**
		 * This method returns the X position of an object that is closest to the
		 * centerX point of a Tile.
		 * 
		 * @param gameObject
		 * @param tile
		 * @return int
		...
		/**
		 * This method returns the Y position of an object that is closest to the
		 * centerY point of a Tile.
		 * 
		 * @param gameObject
		 * @param tile
		 * @return int
		...
		/**
		 * This method calculates which side of a Tile is hit, based on angle between
		 * GameObject and Tile.
		 * 
		 * @param angleGameObjectToTile
		 * @return int
		...
		private static int calculateCollidedTileSide(int angleGameObjectToTile) {
			...
			else if ((angleGameObjectToTile >= 316 && angleGameObjectToTile <= 360)
					|| angleGameObjectToTile >= 0 && angleGameObjectToTile <= 45)
		...
		/**
		 * This method checks if a GameObject has collided with a Tile, and returns this
		 * Tile.
		 * 
		 * @param gameObject
		 * @param tile
		 ...
		 * @return Vector
		 */
		private static Vector checkCollisionBetweenTileAndObject(GameObject gameObject, Tile tile,
				Rectangle collidingTile, Rectangle collidingGameobject) {
		...
		/**
		 * This method checks if a GameObject has collided with another GameObject and
		 ...
		 * @return Vector
		 */
		private static Vector checkCollisionBetweenObjects(GameObject gameObject, GameObject otherGameObject,
				Rectangle gameObjectRectangle, Rectangle otherGameObjectRectangle) {
			Vector objects = new Vector();
